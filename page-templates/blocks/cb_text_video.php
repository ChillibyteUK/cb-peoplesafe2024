...
?>    
<!-- text_video -->
<section class="text_video py-5 bg--<?=$theme?> <?=$extra_classes?>">
    <div class="container">
        <?php
        ...
        <div class="row">
            ...
            <div class="<?=$cols_image?> text_video__container <?=$vid_margin?> text-center <?=$order_image?>">
                ...
                if (get_field('content_below_video')) {
                    ?>
                <div class="text-end pt-5"><?=get_field('content_below_video')?></div>
                    <?php
                }
                ?>
                            ...
                            <div class="modal-body">
                                <div class="embed-responsive embed-responsive-16by9">
                                    ...
                                    if (get_field('video_provider') == 'YouTube') {
                                        ?>
                                    <iframe id="vid<?=$modal?>" class="embed-responsive-item" src="https://www.youtube-nocookie.com/embed/<?=get_field('video_id')?>?autoplay=1" allow="autoplay; fullscreen; picture-in-picture" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
                                        <?php
                                    }
                                    else {
                                        ?>
...
margin-left: 1rem;
    ...
    <div class="pt-4">
        <div class="collapsed h4" data-toggle="collapse" id="header_transcript" data-target="#collapse_transcript" aria-expanded="false" aria-controls="collapse_transcript">Video Transcript</div>
        <div class="collapse" id="collapse_transcript" aria-labelledby="heading_transcript"><div class="pt-2"><?=apply_filters('the_content',get_field('video_transcript'))?></div></div>
    </div>
            <?php
        }
        ?>
